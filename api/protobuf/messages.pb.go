...
type Item struct {
	...
	// Grouping key of the item. Items with the same key are grouped and returned together to the same receiver when the first grouped item hits the head.
	...
	// Grouping takes precedence over priority.
...
// EnqueueRequest contains the item to enqueue and an optional timeout for the operation.
...
// EnqueueResponse contains the server-assigned ID of the enqueued item.
...
// PullSingleRequest specifies how long to wait for an item if the queue is empty.
...
// PullSingleResponse contains the item pulled from the queue.
...
// PullRequest specifies the number of items to pull and how long to wait if the queue is empty.
...
// PullResponse contains the items pulled from the queue.
...
// CommitSingleResponse indicates whether the item was committed.
...
// CommitRequest commits multiple items by their IDs.
...
// RequeueSingleRequest requeues a single item.
...
// RequeueSingleResponse indicates whether the item was requeued.
...
// RequeueRequest requeues multiple items.
...
// RequeueResponse contains the number of items that were requeued.
...
// DeleteSingleRequest deletes a single item by ID.
...
// DeleteSingleResponse indicates whether the item was deleted.
...
// DeleteRequest deletes multiple items by their IDs.
...
// DeleteResponse contains the number of items that were deleted.
...
// CreateQueueRequest contains the parameters to create a new queue.
...
// ListQueuesResponse contains all queues defined on the server.
...
// ListQueueItem contains information about a single queue.
...
type StorageSetRequest struct {
	...
	// Optional time-to-live duration. The item expires after this duration. Omit or set to 0 for no expiration.
